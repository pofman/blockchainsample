...
contract UserAddressRegistry {
    ...
    function registerName(bytes32 name) public returns (bool) {
        ...
        addressMap[msg.sender].lastUpdated = block.timestamp;
        return true;
    }
    ...
    function getByIndex(uint256 index) public view returns (address, bytes32, uint256) {
        if (index >= addresses.length) return (address(0), "", 0);
    ...
    function getByAddress(address addr) public view returns (address, bytes32, uint256) {
        return (addr, addressMap[addr].name, addressMap[addr].lastUpdated);
    }
