...
contract('MultiNumberBettingV6', function(accounts) {
  ...
  it("should assert true", function() {
    ...
    return MultiNumberBettingV6.deployed().then((instance) => {
      multi_number_betting_v6 = instance;
      return web3.eth.getBalance(johns_address);
    }).then(function(result){
      console.log('John balance before initial send : ',web3.utils.fromWei(result,'ether'));

      // #2
      var sendValue = web3.utils.toWei('20','ether');
      return web3.eth.sendTransaction({from:johns_address,to:multi_number_betting_v6.address, value:sendValue});
    }).then((result) => web3.eth.getBalance(johns_address)).then(function(result){
      ...
      // #3 If this is called with any address other than John - it will throw excep
